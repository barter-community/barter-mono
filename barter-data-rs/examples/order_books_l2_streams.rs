...
async fn main() {
    ...
    let trade_stream = Streams::<PublicTrades>::builder().subscribe_bt(
        [(
            BinanceSpot::default(),
            "eth",
            "usdt",
    ...
    let order_book_streams = Streams::<OrderBooksL2>::builder()
    ...
    // let mut binance_stream = order_book_streams.select(ExchangeId::BinanceSpot).unwrap();
